...
if(isset($_FILES['ads_images']) && $_FILES['ads_images']['name']!=""){
  ...
  try {
            ...
            $req= $bdd->prepare('UPDATE `ads` SET `ad_title`=?,`ad_description`=? ,category_id=? ,`general_cat`=?,`suub_cat`=?,state=?,`cities`=?,`city_area`=?,`address_ad`=?,lat=?,lon=?,keyword=? WHERE ads_id=?');
            $req->execute(array(
